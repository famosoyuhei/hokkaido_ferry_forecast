...
GitHub API でワークフローの停止理由を確認できます。

```bash
gh api repos/famosoyuhei/hokkaido_ferry_forecast/actions/workflows/data-collection.yml --jq '.state'
```

| 返ってくる値 | 意味 | 対処 |
|------------|------|------|
| `active` | 正常稼働中 | 対応不要 |
| `disabled_inactivity` | **GitHubが60日非活性で自動停止** → 問題あり | → ステップ1へ（再有効化） |
| `disabled_manually` | **誰かが手動で意図的に停止** | → 意図的か確認してから再有効化 |
...
2. 停止しているワークフローをクリック
3. バナーの文言を確認：
   - **「This workflow was disabled because the repository had no activity」** → `disabled_inactivity`（自動停止）
   - **「This workflow was disabled manually」** → `disabled_manually`（手動停止）
