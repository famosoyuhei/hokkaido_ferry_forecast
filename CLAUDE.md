...
#### 6. 航路リストをハードコードする（2026-05-31 追加）

```python
# ❌ 禁止: スクリプト内で航路リストを直書き
ferry_routes = ['wakkanai_oshidomari', 'wakkanai_kafuka', ...]
# 問題点:
# - 夏季限定の沓形-香深便が入らない（または誤って通年入る）
# - 存在しないキー（wakkanai_kutsugata 等）が紛れ込む
# - 年が変わっても誰も気づかず古いリストが使われ続ける

# ✅ 正解: 日付を渡して時刻表から動的取得
from jst_utils import get_active_routes_on
routes = get_active_routes_on(forecast_date)  # その日の運航便だけ返す
```

#### 7. 時刻表ファイルのパスに年をハードコードする（2026-05-31 追加）

```python
# ❌ 禁止: 年を直書き（2027年になると壊れる）
path = ref_dir / 'heartland_2026_timetable.json'
timetable = self._load_2026_timetable()

# ✅ 正解: year 変数＋ glob フォールバック
...
# または jst_utils の get_timetable_sailings / get_active_routes_on をインポートして使う
```

#### 8. 存在しない航路キーを書く（2026-05-31 追加）

```python
# ❌ 絶対禁止: 稚内-沓形の直行便は存在しない
'wakkanai_kutsugata'
'kutsugata_wakkanai'

# ✅ 正しい沓形関連キー（夏季 6/1〜9/30 のみ）
'kutsugata_kafuka'   # 沓形→香深
'kafuka_kutsugata'   # 香深→沓形
```

**コミット前の禁止パターン検出コマンド（全部0件でOK）**:
```bash
